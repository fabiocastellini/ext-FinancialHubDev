...
import { getVal, getCost} from '../utils/calculations.js';

// Chart is assumed to be loaded globally by Chart.js.
// If Chart.js is also an ES module in your project, import it instead:
...
export function renderOverview() {
  ...
  const liqHoldings = state.holdings.filter(
    h => ['bank', 'cash'].includes(h.type)
  );

  const invTotal = invHoldings.reduce(
    (s, h) => s + getVal(h),
    0
  );

  const liqTotal = liqHoldings.reduce(
    (s, h) => s + h.avg_cost,
    0
  );

  const invCost = invHoldings.reduce(
    (s, h) => s + getCost(h),
    0
  );

  const invGain = invTotal - invCost;
  const invGainPct = invCost > 0
    ? invGain / invCost * 100
    : 0;
  ...
  if (pvEl) {
    pvEl.textContent = fmt(invTotal);
  }

  if (pcEl) {
    pcEl.className =
      'ov-hero-change' + (invGain < 0 ? ' neg' : '');
...
function renderTrendChart() {
  ...
  const todayVal = state.holdings.reduce(
    (s, h) => s + getVal(h),
    0
  );
  ...
  if (
    !points.find(p => p.snapshot_date === todayStr) &&
    todayVal > 0
  ) {
    points.push({
      snapshot_date: todayStr,
      total_value: todayVal
    });
  }
  ...
  if (changeEl) {
    if (points.length >= 2) {
      ...
      const up = diff >= 0;
      const label =
        periodLabels[activePeriod] || 'vs period start';

      changeEl.className =
        'ov-hero-change' + (up ? '' : ' neg');
      ...
      changeEl.innerHTML =
        '<i class="ti ti-minus"></i> No history yet';
    }
  }


  const labels = points.map(
    p => new Date(p.snapshot_date).toLocaleDateString(
      'en-GB',
      {
        day: '2-digit',
        month: 'short'
  ...
  const labelEl =
    document.getElementById('chart-range-label');
  ...
  if (points.length > 0) {
    ...
    const last = fmtD(
      points[points.length - 1].snapshot_date
    );
    ...
    labelEl.textContent =
      'No historical data yet — data builds up daily';
  }
}
...
function renderHoldingBoxes() {
  ...
  const sorted =
    Object.keys(byType).sort(
      (a, b) =>
        (TYPE_LABELS[a] || a)
          .localeCompare(TYPE_LABELS[b] || b)
    );


  const grid =
    document.getElementById('holding-grid');
  ...
  if (!sorted.length) {
    grid.innerHTML =
      '<div class="empty" style="grid-column:1/-1;padding:2rem">' +
        ...
        'margin-bottom:8px;color:var(--text3)"></i>' +
        '<p>No holdings yet</p>' +
      '</div>';
  ...
  // on desktop use equal-width grid columns
  ...
  grid.innerHTML = sorted.map(type => {
    ...
    const icon =
      TYPE_ICONS[type] || 'ti-wallet';

    const label =
      TYPE_LABELS[type] || type;

    return `
      <div class="holding-box type-${type}">
        <div class="holding-box-ghost">
          <i class="ti ${icon}" style="color:var(--tc)"></i>
        </div>

        <div class="holding-box-label">
          ${label}
        </div>

        <div class="holding-box-value">
          ${fmt(val)}
        </div>

        <div class="holding-box-pct">
          ${pct}% of portfolio
        </div>
      </div>
    `;
